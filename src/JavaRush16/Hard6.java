...
public class Hard6 {
    ...
    public static int calculateHorsesFinished(List<Horse> horses) throws InterruptedException {
        ...
        for (Horse horse : horses){
            if(horse.isFinished()) // Если лошадь финишировала
                finishedCount++;  // то считаем её
            else{                         // иначе выводим сообщение в консоль
                System.out.println("Waiting for " + horse.getName());
                horse.join();         // ждём, пока лошадь не финиширует
            }
        }
